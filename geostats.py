    ...
    def _worker(
        ...
        while True:
            ...
            interp_cov = geostruct.covariance_points(
                ix, iy, point_data.loc[pt_names, "x"], point_data.loc[pt_names, "y"]
            )
...
class ExpVario(Vario2d):
    ...
    def _h_function(self, h):
        ...
        a = self.a
        return self._h_function_calc(contribution,a, h)

    @staticmethod
    @jit(nopython=True)
    def _h_function_calc(contribution,a, h):

        return contribution * np.exp(-1.0 * h / a)
